// Package demo is the race console's backend — the 45 seconds this project is
// judged on (CLAUDE.md, demo priorities #1).
//
// It fires N genuinely concurrent booking attempts at one facility and one slot,
// then reads the answer back out of the database. It is a product surface, not a
// script: the same code serves POST /api/v1/demo/race, cmd/racedemo, and the
// /race console in the web app.
//
// # Why this package exists at all, rather than living in test/
//
// The barrier runner below is the SAME code path test/testutil.Race exercises —
// testutil.Race is a thin wrapper over demo.Race. Two implementations of
// "release N goroutines together" would eventually drift, and the day they drift
// is the day the demo proves something different from what CI proves.
//
// It lives here rather than in test/testutil because testutil pulls in
// testcontainers and the `testing` package, neither of which belongs in the API
// binary or in a CLI a judge might run.
//
// # Zero external dependencies
//
// Nothing in this package touches Redis, the read replica, the HTTP stack, or
// any network beyond the local Postgres connection. Venue wifi cannot break it
// (IMPLEMENTATION.md §13, risk register §18).
package demo
...
// ErrNoAttempts is returned when Race is asked for a non-positive n.
var ErrNoAttempts = errors.New("demo: race needs n > 0")
...
//
// Every goroutine is joined before Race returns, so it leaks nothing even when
// fn blocks until ctx expires.
//
// Callers should warm the connection pool first, or the race measures connection
// setup rather than contention.
func Race(ctx context.Context, n int, fn func(ctx context.Context, i int) (any, error)) (Outcome, error) {
	if n <= 0 {
		return Outcome{}, fmt.Errorf("%w, got %d", ErrNoAttempts, n)
	}
	...
	return newOutcome(attempts, elapsed), nil
}
...
// Summarise renders the outcome for logs — the shape the race demo prints.
